    ...
    },
    {
        ...
        fields: [
            ...
            { key: "extraHeaders", label: "额外请求头", kind: "textarea", envNames: ["VOZEB_PRO_PAYPLY_EXTRA_HEADERS", "PAYPLY_EXTRA_HEADERS"], placeholder: '{"x-channel":"vozeb-pro"}', note: "填写 JSON 对象，值会随下单请求发送。", advanced: true },
            {
                ...
                note: "留空使用 MOCREAI 默认字段；支持 {{orderId}}、{{orderNo}}、{{amount}}、{{amountCents}}、{{currency}}、{{notifyUrl}}、{{returnUrl}}、{{cancelUrl}} 等变量。",
                advanced: true,
            },
            ...
            },
            {
                ...
                note: "留空使用 MOCREAI 默认字段；支持 {{orderId}}、{{orderNo}}、{{providerOrderId}}、{{providerPaymentId}}、{{amount}}、{{amountCents}}、{{currency}}、{{reason}} 等变量。",
                advanced: true,
            },
            ...
            { key: "webhookOrderIdField", label: "MOCREAI 订单 ID 字段", kind: "text", envNames: ["VOZEB_PRO_PAYPLY_WEBHOOK_ORDER_ID_FIELD"], placeholder: "metadata.vozebProOrderId", advanced: true },
            { key: "webhookOrderNoField", label: "MOCREAI 订单号字段", kind: "text", envNames: ["VOZEB_PRO_PAYPLY_WEBHOOK_ORDER_NO_FIELD"], placeholder: "metadata.vozebProOrderNo", advanced: true },
