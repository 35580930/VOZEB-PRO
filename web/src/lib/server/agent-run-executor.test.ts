...
describe("executeAgentRun backend settings", () => {
    ...
    it("turns explicit canvas text-node content into a node result without calling the text task API", async () => {
        mocks.run = runWithTasks([
            {
                ...
                title: "欢迎文案",
                type: "text",
                prompt: "创建一个文字节点，内容写“欢迎使用 MOCREAI Agent”，放在画布中央，并选中它。\n\n严格输出要求：只输出最终文本，不要标题、Markdown、解释或列表。",
                count: 1,
                dependencies: [],
                status: "ready",
                attempts: 0,
        ...
        expect(mocks.run?.tasks[0].result).toEqual({ content: "欢迎使用 MOCREAI Agent" });
        ...
        expect(completed?.data?.ops).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    ...
                    nodeType: "text",
                    position: { x: 800, y: 96 },
                    metadata: expect.objectContaining({ content: "欢迎使用 MOCREAI Agent" }),
                }),
